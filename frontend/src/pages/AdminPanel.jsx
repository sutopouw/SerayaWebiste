...
function AdminPanel() {
  ...
  const addEvent = async () => {
    ...
    try {
      // eventDate dari input datetime-local sudah dalam waktu lokal
      ...
      console.log('Event added:', {
        input_date: eventDate,
        local_date: localDate.toLocaleString(),
        utc_date: localDate.toISOString(),
        server_response: data
      });

      toast.success(`Event berhasil ditambahkan untuk ${data.eventDateFormatted}`);
            ...
            <ul className="space-y-3">
              {winners.map((winner) => (
                <li key={winner.id} className="p-3 bg-gray-50 rounded-lg text-gray-700">
                  <span className="font-semibold">{winner.winner_username}</span> 
                  <br />
                  Discord ID: {winner.discord_id}
                  <br />
                  Role: <span className="text-blue-600">{winner.role_reward}</span>
                  <br />
                  Waktu: {winner.created_at_formatted || new Date(winner.created_at).toLocaleString('id-ID', {
